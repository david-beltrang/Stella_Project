...
/**
 * Implementación JDBC para la persistencia de las estadísticas de usuario
 * (racha, pescaditos).
 */
public class UsuarioStatsRepository implements InterfazUsuarioStatsRepository {
    ...
    // Usar comillas dobles para que H2 respete los nombres case-sensitive
    private static final String SQL_UPDATE_RACHA = "UPDATE \"usuario_stats\" SET racha_dias = racha_dias + ? WHERE usuario_id = ?";
    private static final String SQL_UPDATE_PESCADITOS = "UPDATE \"usuario_stats\" SET pescaditos = pescaditos + ? WHERE usuario_id = ?";
    ...
    public void actualizarRacha(int usuarioId, int diasSumar) {
        try (Connection conn = connMgr.getConnection();
            ...
            if (affectedRows == 0) {
                ...
                logger.warn(
                        "No se encontró registro de stats para actualizar racha del usuario {}. La fila debe ser creada primero.",
                        usuarioId);
            }
    ...
    @Override
    public void crearStatsIniciales(int usuarioId) {
        String sql = "INSERT INTO \"usuario_stats\" (usuario_id, pescaditos, objetivo_sesiones, racha_dias, tiempo_total_estudio_segundos) VALUES (?, 0, 1, 0, 0)";
        try (Connection conn = connMgr.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, usuarioId);
            ps.executeUpdate();
        } catch (SQLException e) {
            logger.error("Error al crear stats iniciales para usuario {}", usuarioId, e);
            throw new RuntimeException("Error al crear stats iniciales", e);
        }
    }
}